...
from cv.gestures import Gesture, GestureClassifier, GestureResult, TemporalGestureConfirmer, TwoHandPainCounter
...
def hand(extended: set[int], thumb: str | None = None) -> HandLandmarks:
    ...
    points[5] = Point(0.45, 0.65)
    points[9] = Point(0.55, 0.65)
    points[2] = Point(0.25, 0.45)
    if thumb is not None:
        points[3] = Point(0.22, 0.28)
...
class GestureTests(unittest.TestCase):
    ...
    def test_two_hand_pain_counter_combines_hands(self):
        counter = TwoHandPainCounter()
        result = counter.count([hand({0, 1}), hand({0, 1, 2})], timestamp=5)
        self.assertEqual(result.count, 5)
        self.assertGreaterEqual(result.confidence, 0.65)

    def test_open_hands_include_thumbs_for_pain_count(self):
        result = TwoHandPainCounter().count([hand({0, 1, 2, 3}, "up")], timestamp=6)
        self.assertEqual(result.count, 5)
